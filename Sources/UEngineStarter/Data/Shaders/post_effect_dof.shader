...
float focalDepth = 10.0f;  //external focal point value, but you may use autofocus option below
...
float range = 0.005; //focal range
float maxblur = 100.25; //clamp value of max blur
...
float bdepth(vec2 coords) //blurring depth
{
	...
	for( int i=0; i<9; i++ )
	{
		float tmp = texture2D(depthScene, coords + offset[i]).r;
		d += tmp * kernel[i];
	}
...
vec3 color(vec2 coords,float blur) //processing the sample
{
	...
	col.r = texture2D(colorScene,coords + vec2(0.0,1.0)*texel*fringe*blur).r;
	col.g = texture2D(colorScene,coords + vec2(-0.866,-0.5)*texel*fringe*blur).g;
	col.b = texture2D(colorScene,coords + vec2(0.866,-0.5)*texel*fringe*blur).b;
...
void main() 
{
	...
	float depth = texture2D(depthScene,Vert.texcoord).x;
	float blur = 0.0;
	
	if (depthblur)
	{
		depth = bdepth(Vert.texcoord);
	}
	...
	if (autofocus)
	{
		float fDepth = texture2D(depthScene,focus).x;
		blur = clamp((abs(depth - fDepth)/range)*100.0,-maxblur,maxblur);
	}
	
	vec2 noise = rand(Vert.texcoord)*namount*blur;
	
	float w = (1.0/width)*blur+noise.x;
	float h = (1.0/height)*blur+noise.y;
	
	vec3 col = texture2D(colorScene, Vert.texcoord).rgb;
	float s = 1.0;
	...
	{   
		...
		for (int j = 0 ; j < ringsamples ; j += 1)   
		{
			...
			col += color(Vert.texcoord + vec2(pw*w,ph*h),blur)*mix(1.0,(float(i))/(float(rings)),bias)*p;  
