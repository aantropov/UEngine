...
namespace MyGUI
{
	...
	UEngineRTTexture::UEngineRTTexture(unsigned int _texture) :
		mTextureID(_texture),
		mWidth(0),
		mHeight(0),
		mFBOID(0),
	...
	UEngineRTTexture::~UEngineRTTexture()
	{
		if (mFBOID != 0)
		{
	...
	void UEngineRTTexture::begin()
	{
		...
		UEngineRenderManager::getInstance().begin();
		glOrtho(-1, 1, 1, -1, -1, 1);
		glClear(GL_COLOR_BUFFER_BIT/* | GL_DEPTH_BUFFER_BIT*/);
	}

	void UEngineRTTexture::end()
	{
		UEngineRenderManager::getInstance().end();
	...
	void UEngineRTTexture::doRender(IVertexBuffer* _buffer, ITexture* _texture, size_t _count)
	{
		UEngineRenderManager::getInstance().doRender(_buffer, _texture, _count);
	}
